//============================================================================
// Name		: TrackAttributeSuccess.cpp
// Author	  :
// Version	 :
...
int main(int argc, char **argv) {
	...
	CBAttrInfoRef roomAttributeChangedCB=nullptr;
	...
	CBClientInfoRef roomJoinedCB = unionClient.Self()->NotifyClientInfo::AddEventListener(Event::JOIN_ROOM,
		[&meetingRoom,&loginAttempt,&loginSucceed, &roomAttributeChangedCB](EventType e, ClientID clientID, ClientRef cr, int, RoomID roomID, RoomRef  rr, UPCStatus s) {
		...
		if (roomID == lobbyName) {
			...
			meetingRoom = rr;
			loginSucceed = true;
			cerr << "Adding room attribute listeners" << endl;
			roomAttributeChangedCB = meetingRoom->NotifyAttrInfo::AddEventListener(Event::UPDATE_CLIENT_ATTRIBUTE,
				[](EventType t, AttrName n, AttrVal v, AttrScope s, AttrVal ov, ClientRef c, UPCStatus status) {
					cerr << "room attrib " << n << " -> " << v << " in scope " << s << " for client " << (c ? c->GetName() : "senki") << endl;
			});
	...
	// ------------------------
	//
	// Before we log in to the lobby, and then into the prezentation, we add attribute listeners
	// for the most part, we are interested in client attributes
	//

	cerr << "Adding client attribute listeners" << endl;
	CBAttrInfoRef attributeChangedCB = unionClient.Self()->NotifyAttrInfo::AddEventListener(Event::UPDATE_ATTR,
		[](EventType t, AttrName n, AttrVal v, AttrScope s, AttrVal ov, ClientRef c, UPCStatus status) {
			cerr << "client attrib " << n << " -> " << v << " in scope " << s << " for client " << (c ? c->GetName() : "senki") << endl;
	});
