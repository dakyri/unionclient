...
int main(int argc, char **argv) {
	...
	CBClientInfoRef roomJoinedCB = unionClient.Self()->NotifyClientInfo::AddEventListener(Event::JOIN_ROOM,
		...
		if (roomID == lobbyName) {
			...
			// STEP 7
			//
			// add occupant listeners. this piece of code is the last called on a successful login
			// any changes to the occupancy of the meeting room are now tracked
			//
			cerr << "Adding occupant tracking listeners" << endl;
	...
	removeMeetingOccupantCB = nullptr;
	addMeetingOccupantCB = nullptr;
